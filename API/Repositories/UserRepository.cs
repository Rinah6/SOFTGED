...
namespace API.Repositories
{
    public class UserRepository
    {
        ...
        public async Task<UserDetails?> GetUserDetails(Guid userId)
        {
            ...
            using var cmd = new SqlCommand(@"
                SELECT u.Username, u.FirstName, u.LastName, u.Email, u.RoleId, u.ProjectId
                FROM Users AS u
                WHERE u.DeletionDate IS NULL
                AND u.Id = @userId;
            ", conn);
        ...
        public async Task Update(Guid userId, UserToUpdate userToUpdate, UserRole currentUserRole)
        {
            ...
            using var cmd = new SqlCommand(@"
                UPDATE Users SET Username = @username, LastName = @lastName, FirstName = @firstName, Email = @email, Password = @password, RoleId = @role, ProjectId = @projectId
                WHERE Id = @userId;
            ", conn);
